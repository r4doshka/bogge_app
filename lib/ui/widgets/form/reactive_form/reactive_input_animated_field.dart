...
class ReactiveInputAnimatedField<T> extends HookConsumerWidget {
  ...
  const ReactiveInputAnimatedField({
          ...
          builder: (context, constraints) {
            return ReactiveValueListenableBuilder(
              ...
              builder: (context, control, _) {
                final text = control.value?.toString() ?? '';

                final textPainter = TextPainter(
                  text: TextSpan(
                    text: text,
                    style: textStyle ?? text_s17_w500_lsm043,
                  ),
                  maxLines: 1,
                  textDirection: TextDirection.ltr,
                );

                final horizontalPadding = AppSpace.s16.w * 2;
                final availableWidth = constraints.maxWidth - horizontalPadding;

                textPainter.layout(maxWidth: availableWidth);

                final shouldExpand = textPainter.didExceedMaxLines;

                final currentHeight = shouldExpand
                    ? (maxContainerHeight ?? 85.h)
                    : (minContainerHeight ?? containerHeight ?? 54.h);
